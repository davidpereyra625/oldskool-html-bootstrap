(function () {
  ...
  const handleOptionChange = ({ event, option }) => {
    const optionParent = event.target
      ? event.target.closest(`.${PARENT_CSS_CLASS}`)
      : false;
    const selectedOptionLabel = optionParent
      ? optionParent.querySelector(`.${SELECTED_OPTION_CLASS}`)
      : false;
    const selectedOption =
      event.target && event.target.value ? event.target.value : false;
